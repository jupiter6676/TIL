...
#### 3.3.5.1. IP(Internet Protocol)

- 역할

  - 패킷(Packet)이라는 통신 단위로 데이터를 라우팅
...
- IP 패킷
  ...
  - 네트워크 계층에서 데이터 캡슐화 시, 데이터에 IP 헤더를 추가하는데 그 형태를 IP 패킷이라 함
    ...
    - IP 정보는 각 패킷에 첨부되어, 라우터가 패킷을 올바른 곳에 전달하는 데 도움을 줌

  - 패킷이 목적지에 도착되면 IP와 함께 어떤 전송 프로토콜이 사용되는지에 따라 다르게 처리됨
  ...
  - IP 패킷의 구조
    ...
    - ID, Flags, Fragment Offset: **단편화**(Fragmentation)와 **재결합**(Reassembly)을 위해 사용
      - 데이터그램을 잘게 쪼개고 다시 합칠 때 사용됨
      
    - TTL(Time to Live): 데이터가 전달될 수 있는 Hop 수
      - 라우터 하나를 거칠 때마다 Hop 수가 하나씩 줄어듦
      - TTL 값이 0이 되면 라우터에서 데이터를 제거함
      
    - 프로토콜(Protocol): 상위 전송(Transport) 계층의 프로토콜을 표시함 (TCP, UDP 혹은 ICMP)
      - 데이터가 목적지에 도착했을 때만 사용됨
      
    - 헤더 체크섬(Header Checksum): 받은 데이터의 에러 검출을 위해 사용
      - TCP: 신뢰성있는 데이터 전송을 위해, 에러가 있으면 데이터를 재전송
      
      - UDP: 체크섬 계산을 하지만, 에러를 정정하는 기능은 없음
...
- IPv4 주소 체계
  ...
  - 32bit
    ...
    - 모든 호스트에 IP를 할당할 수 없어 IPv6가 등장

    - 8bit 단위(옥텟)로 끊어 10진수로 표기 (0 ~ 255)

      ![image-20240208230859015](Assets/01_OSI_7_Layer.assets/image-20240208230859015.png)
...
- **IP 클래스**
  ...
  ![image-20240208230859017](Assets/01_OSI_7_Layer.assets/image-20240208230859017.png)

  - 예전의 IPv4 주소를 할당하던 방식으로, 현재는 CIDR 방식을 통해 할당하고 있음

  - 종류

    - A 클래스: 대규모 네트워크 주소
      - 앞 8bit는 네트워크 ID로 사용됨 (0 ~ 127)
      - 뒤 24bit는 호스트 ID로 사용됨
      - 가능한 호스트의 수는 16,777,214대
      - 사설 IP 대역
        - `10.0.0.0` ~ `10.255.255.255`
        - 공인 IP 대역은 사설 IP 대역을 뺀 나머지 범위
      
      - B 클래스: 중형 네트워크 주소
        - 앞 16bit는 네트워크 ID로 사용됨 (128 ~ 191)
        - 뒤 16bit는 호스트 ID로 사용됨
        - 가능한 호스트의 수는 6,5534대
        - 사설 IP 대역
          - `172.16.0.0` ~ `172.31.255.255`
          - 공인 IP 대역은 사설 IP 대역을 뺀 나머지 범위
      
      - C 클래스: 소규모 네트워크 주소 (192 ~ 223)
        - 앞 24bit는 네트워크 ID로 사용됨
        - 뒤 8bit는 호스트 ID로 사용됨
        - 가능한 호스트의 수는 2544대
        - 사설 IP 대역
    ...
    - D 클래스: 멀티캐스트 주소 (224 ~ 239)

    - E 클래스: 연구 및 특수 용도 주소 (240 ~ 255)



- 네트워크 주소 & 브로드캐스트 주소
  - 네트워크 주소
    - 작은 그룹의 네트워크 단위를 식별하기 위해 사용되는 IP 주소

    - ex. `192.168.0.0` ~ `192.168.255.255` 범위를 대표하는 네트워크 주소는 `192.168.0.0`
  - 브로드캐스트 주소
    - 네트워크 내의 장비 모두에게 한 번에 데이터를 전송하는 데 사용되는 IP 주소

    - 호스트 ID의 이진수가 `11111111`이 되는 주소

    - ex. C클래스의 경우 맨 뒤의 8bit가 호스트 ID이므로, 브로드캐스트 주소는 `192.168.0.255`
  - 네트워크 주소와 브로드캐스트 주소는 IP 주소로 사용할 수 없음
  - 네트워크 주소와 브로드캐스트 주소 범위의 사이를 할당받아 사용함
  - `xxx.xxx.xxx.1`은 보통 라우터가 할당받게 되어, 사용자는 `xxx.xxx.xxx.2`부터 할당받음



- ==**서브넷**==(Subnet)

  ![image-20240208230859016](Assets/01_OSI_7_Layer.assets/image-20240208230859016.png)

  - IP 주소를 ==클래스 단위보다 더 작은 단위로 분할한 네트워크==
    - 대규모의 네트워크에서는 브로드캐스트 패킷 등의 전송 시, 네트워크가 복잡하여 혼란이 생길 수 있음
      - 따라서 목적에 맞게 더 작은 단위로 분할하여 편리하게 사용하기 위헤 서브네팅
      - 만일 IP가 100개만 필요하다면, C 클래스(256개)를 더 쪼개서 사용할 수 있음
      - 이러한 ==**서브네팅은 서브넷 마스크 계산을 통하여 수행**==됨
      - 클래스 기반의 IP 주소 할당 방식보다 더 효율적임
    - 하나의 서브넷 안에서는 라우터 없이 동일한 LAN(이더넷/Wi-Fi) 안에서 물리적으로 연결되어 있어, 라우터의 중계 없이도 서로 물리적으로 도달할 수 있음
    
  - 필요성
    - 클래스 기반 IP 할당 방식은 비효율적이기에, 서브네팅을 통해 IP를 할당함
      - A 클래스: Host ID의 개수는 약 1,670만 개
      - B 클래스: Host ID의 개수는 65,536개
      - C 클래스: Host ID의 개수는 256개
    - 어느 기업이 IP를 10,000개 정도를 사용해야 한다고 할 때
      - B 클래스를 할당하는 경우, 나머지 50,000여개의 IP는 쓰이지 않은 채 기업에 점유되어 버림
      - C 클래스를 할당하는 경우, IP가 부족한 현상이 발생함
    
  - ==**서브넷 마스크**==(Subnet Mask)
  
    - 네트워크 ID와 호스트 ID를 보다 편하게 구분하기 위해 사용
  
    - 표현
  
      - IP 주소와 동일한 32bit의 2진수로 표현됨
      - IP와 다른 점은 연속된 1과 0으로만 구성되어 있음
      - 서브넷 마스크의 옥텟이 255면 네트워크 ID를 가리키며, IP가 속한 클래스를 간단히 확인할 수 있음
      - ex. `255.255.255.0` → 네트워크 ID가 24bit
    ...
    - ==**CIDR**==(Classless Inter Domain Routing)
      ...
      - 클래스 없는 도메인 간 라우팅 기법
        - 도메인 간의 라우팅에 사용되는 IP 주소를 할당할 때, 클래스 체계를 사용하지 않고 더 능동적으로 할당하는 방식 중 하나
        - CIDR 기법에는 **서브네팅**과 **슈퍼네팅**(여러 개의 서브넷을 하나의 네트워크로 통합하는 작업)이 속함
      - CIDR 표기법
        - 서브넷 마스크 주소의 상위 몇 bit까지 서브넷으로 지정할 것인가를 나타냄
          - `/8`, `/16`, `/24`와 같이 표기
          - ex. `192.168.1.0/24`
            - 서브넷 마스크의 상위 24bit가 1의 값을 가짐
            - → 서브넷 마스크 주소가 `255.255.255.0`임을 의미



- IPv6 주소 체계
  - 128bit
    - 16bit 단위로 끊어 4자리의 16진수로 표현
    - ex. `2001:0db8:85a3:08d3:1319:8a2e:0370:7334`
    - 2^128^개의 IP 주소 할당 가능
  - 장점
    - IPv4의 주소 부족 문제 해결
    - Header 형식이 개선됨에 따라, 우선순위 및 품질에 따른 순차적 할당 등의 QoS 지원
    - 보안 기능을 기본적으로 제공



#### 3.3.5.2. DHCP(Dynamic Host Configuration Protocol)

- ✨✨✨✨✨✨✨ **DHCP**
  - DHCP를 통해 IP 주소를 자동으로 할당할 수 있음
  - 
...
- 프로토콜
  - IP
    - [[네트워크] IP 프로토콜 개념 정리(IPv4, IPv6)](https://seosh817.tistory.com/33)
    - [인터넷 프로토콜이란?](https://www.cloudflare.com/ko-kr/learning/network-layer/internet-protocol/)
    - [[제4장] 네트워크 계층: 데이터 평면](https://velog.io/@jnary/%EC%A0%9C4%EC%9E%A5-%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC-%EA%B3%84%EC%B8%B5-%EB%8D%B0%EC%9D%B4%ED%84%B0-%ED%8F%89%EB%A9%B4)
    - [🌐 IP 클래스 · 서브넷 마스크 · 서브넷팅 계산법 💯 총정리](https://inpa.tistory.com/entry/WEB-IP-%ED%81%B4%EB%9E%98%EC%8A%A4-%EC%84%9C%EB%B8%8C%EB%84%B7-%EB%A7%88%EC%8A%A4%ED%81%AC-%EC%84%9C%EB%B8%8C%EB%84%B7%ED%8C%85-%EC%B4%9D%EC%A0%95%EB%A6%AC)
