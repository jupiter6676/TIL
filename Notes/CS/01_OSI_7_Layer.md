...
- ==**서브넷**==(Subnet)
  ...
  - IP 주소를 ==클래스 단위보다 더 작은 단위로 분할한 네트워크==
    - 대규모의 네트워크에서는 브로드캐스트 패킷 등의 전송 시, 네트워크가 복잡하여 혼란이 생길 수 있음
      ...
      - 이러한 서브네팅은 서브넷 마스크 계산을 통하여 수행됨
    - ✨✨✨ 하나의 서브넷 안에서는 라우터 없이, 동일한 LAN(이더넷/Wi-Fi) 안에서 물리적으로 연결되어 있음
  ...
  - ==**서브넷 마스크**==(Subnet Mask)
    ...
    - 표현
      ...
      - 서브넷 마스크의 옥텟이 255면 네트워크 ID를 가리킴
    ...
    - ✨✨✨✨✨ CIDR
      ...
      - 클래스 기반의 IP 주소 할당 방식보다 더 효율적
      - IP 주소의 상위 몇 bit까지 서브넷으로 지정할 것인가를 나타냄
      - ex. `192.168.1.0/24`는 상위 24bit까지 서브넷으로 
