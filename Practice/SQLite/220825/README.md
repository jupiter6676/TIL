...
for m in movies:
    print(m.director.name, m.genre.title, m.title, m.opening_date, m.running_time, m.screening)
    # print(director.name, m.genre.title, m.title, m.opening_date, m.running_time, m.screening)
```
...
for m in movies:
    if m.running_time > 119:
        director = Director.objects.get(id=m.director_id)
...
# 감독이 봉준호, 장르가 드라마인 영화들
movies = Movie.objects.filter(director_id=director.id, genre_id=genre.id).order_by('opening_date')
...
> 참고 사이트 [[Django] QuerySet 메소드 정리 (2) - CRUD](https://devvvyang.tistory.com/37)
> 
